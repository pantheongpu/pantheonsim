...
for f in tools/characterize-hip.cpp tools/characterize.cu; do
...
scp -i "$key_file" -q -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null \
    -o LogLevel=ERROR "$here/tools/characterize-hip.cpp" root@"$IP":/tmp/ 2>/dev/null
...
if grep -q "Compute Unit" "$outdir/${size}.rocminfo.txt" 2>/dev/null; then
  ...
  echo "[$size] now run: tools/rocminfo-to-profile.py $outdir/${size}.rocminfo.txt"
else

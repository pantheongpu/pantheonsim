...
// (nvidia/registers/measurements/), and its BAR0 as far as it was measured
// (nvidia/registers/mmio.yaml).
#include <cstring>
#include <string>
...
// The cards whose BAR0 has been measured, by PCI device ID.
bool measured_bar0(const telemetry::DeviceSample& d) {
  return std::strcmp(d.vendor, "amd") != 0 && (d.pci_device_id >> 16) == 0x2208;   // RTX 3080 Ti
}
...
const Vendor& nvidia() {
  ...
  static const Vendor v = {
      ...
      16u << 20,  0xbadf5040u,       measured_bar0, nvidia_bars,

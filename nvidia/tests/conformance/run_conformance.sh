...
#
#   nvidia/tests/conformance/run_conformance.sh            # both sides (needs a GPU)
#   VGPU_ONLY=1 nvidia/tests/conformance/run_conformance.sh  # virtual side only
#
...
fail=0
for src in "$root"/nvidia/tests/conformance/*.cu; do
  ...
  if grep -q "nvjpeg" "$src"; then
    libs="$libs -lnvjpeg"
    inc="$inc -DVGPU_DATA_DIR=\"$root/nvidia/tests/conformance/data\""
  fi
  ...
  if grep -q "nccl" "$src"; then
    ...
    inc="$inc -I$root/nvidia/third_party/nccl_include"
    libs="$libs -lnccl"
  ...
  elif python3 "$root/nvidia/tests/conformance/compare_numeric.py" \

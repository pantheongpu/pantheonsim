#!/usr/bin/env bash
# A cooperative launch through cudaLaunchKernelEx, and a library splicing its own
# nodes into a stream capture through cudaStreamGetCaptureInfo and
# cudaStreamUpdateCaptureDependencies. Skips if nvcc is unavailable.
set -euo pipefail
root="$(cd "$(dirname "$0")/../../.." && pwd)"
. "$root/tests/shim_guard.sh"
shim="${VGPU_BUILD_DIR:-$root/build}/shim"
src="$root/nvidia/tests/e2e/capture_splice.cu"
out="${TMPDIR:-/tmp}/vgpu_e2e_capture_splice_$$"
...
# cooperative_groups needs C++17 under CUDA 13.
...
rm -f "$out"
echo "cooperative Ex launch and capture splicing: $result"
[[ "$result" == "PASS" ]]

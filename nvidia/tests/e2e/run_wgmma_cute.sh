...
if [[ ! -f "$cutlass/include/cute/tensor.hpp" ]]; then
  if [[ -n "${CUTLASS_DIR:-}" ]]; then
    echo "FAIL: CUTLASS_DIR=$CUTLASS_DIR has no include/cute/tensor.hpp"; exit 1
  fi
  tmp="$cutlass.tmp.$$"
  mkdir -p "$tmp"
  # Only the headers: the release is mostly examples and tools.
  if ! curl -fsSL "https://github.com/NVIDIA/cutlass/archive/refs/tags/$cutlass_tag.tar.gz" |
       tar -xz -C "$tmp" --strip-components=1 --no-wildcards-match-slash --wildcards 'cutlass-*/include/*' 'cutlass-*/LICENSE.txt' 2>/dev/null; then
    rm -rf "$tmp"

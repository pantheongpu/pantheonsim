#!/usr/bin/env bash
# Layered and cubemap textures and layered surfaces, bit for bit against an RTX 3060 (texture_layers.cu).
set -uo pipefail
root="$(cd "$(dirname "$0")/../../.." && pwd)"
. "$root/tests/shim_guard.sh"
shim="${VGPU_BUILD_DIR:-$root/build}/shim"
out="${TMPDIR:-/tmp}/vgpu-texture-layers.$$"
shopt -s nullglob
...
"$nvcc_bin" -std=c++17 -arch=sm_86 -cudart shared -Wno-deprecated-gpu-targets "${san_flags[@]}" \
     "$root/nvidia/tests/e2e/texture_layers.cu" -o "$out" || { echo "FAIL: does not compile"; exit 1; }
...
rc=$?
sed 's/^/    /' "$log"
[[ $rc == 0 && "$(tail -n 1 "$log")" == PASS ]] || { echo "FAIL: layered and cubemap textures"; exit 1; }

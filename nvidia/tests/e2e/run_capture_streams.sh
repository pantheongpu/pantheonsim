#!/usr/bin/env bash
# Stream capture across streams -- a fork and a join into one graph, and the
# documented refusals -- and host functions, external events and stream-ordered
# allocation on a capturing stream. Skips if nvcc is unavailable.
set -euo pipefail
root="$(cd "$(dirname "$0")/../../.." && pwd)"
. "$root/tests/shim_guard.sh"
shim="${VGPU_BUILD_DIR:-$root/build}/shim"
src="$root/nvidia/tests/e2e/capture_streams.cu"
out="${TMPDIR:-/tmp}/vgpu_e2e_capture_streams_$$"
...
# C++17, as the other e2e sources are compiled.
...
rm -f "$out"
echo "capture across streams: $result"
[[ "$result" == "PASS" ]]

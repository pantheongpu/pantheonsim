#!/usr/bin/env bash
# Every WMMA shape and element type through CUDA's mma.h API (wmma_types.cu),
# each loaded, multiplied, stored and compared exactly with the host. Skips
# when nvcc cannot target sm_80 (the f64, tf32, bf16 and b1 .and forms need
# it).
set -euo pipefail
...
shim="$build/shim"
src="$root/nvidia/tests/e2e/wmma_types.cu"
out="${TMPDIR:-/tmp}/vgpu_e2e_wmma_types_$$"
...
if ! nvcc -std=c++17 -O1 -cudart shared -arch=compute_80 -code=compute_80 \
       $(shim_sanitizer_nvcc_flags "$shim") "$src" -o "$out" 2>"$out.log"; then
  if grep -q "compute_80" "$out.log"; then
    rm -f "$out.log"; echo "SKIP: this nvcc cannot target sm_80"; exit 0
  fi

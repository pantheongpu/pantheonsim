...
#
#   nvidia/tools/characterize-aws.sh <instance-type> [region]
#
...
for f in nvidia/tools/characterize.cu nvidia/tools/characterize-telemetry.sh \
         nvidia/tests/conformance/ptx_semantics.cu nvidia/tests/conformance/control_flow.cu; do
...
scp -i "$key_file" -q -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o LogLevel=ERROR \
    "$here/nvidia/tools/characterize.cu" "$here/nvidia/tools/characterize-telemetry.sh" \
    "$here/nvidia/tests/conformance/ptx_semantics.cu" "$here/nvidia/tests/conformance/control_flow.cu" \

...
trap 'rm -rf "$tmp"' EXIT
fail=0
fixtures="vector_add ops math memory globals grid bytes int64 atomics mixed half calls builtins packed"
...
( cd "$tmp" && cat ops.c math.c memory.c globals.c grid.c bytes.c int64.c atomics.c mixed.c half.c calls.c builtins.c packed.c > both.c &&
